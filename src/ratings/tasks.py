...
def generate_fake_ratings(count=100, users=10, null_avg=False):
    user_s = User.objects.first()
    user_e = User.objects.last()
    random_user_id = random.sample(range(user_s.id, user_e.id), users)
    users = User.objects.filter(id__in=random_user_id)
